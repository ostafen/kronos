...
import {
  ...
  BreadcrumbRoot,
} from '@/components/chakra/breadcrumb.tsx';
    ...
    <>
      ...
      <Flex direction="column" mt="8" gap="3">
        ...
        <Flex gap="2" mt="3" alignItems="center" flexWrap="wrap">
          <Badge title="Schedule unique id" colorPalette="orange">
            {id}
          </Badge>
          ...
          {isRecurring && (
            <>
              ...
              {cronExpr && (
                <Badge title="Cron expression" colorPalette="red">
                  {cronExpr}
                </Badge>
              )}
            </>
          )}
          <Badge title="Webhook URL" colorPalette="pink">
            {url}
          </Badge>
            ...
            <>
              {startAt && (
                <Badge colorPalette="cyan">
                  Start at {formatDate(startAt)}
                </Badge>
              )}
              {endAt && (
                <Badge colorPalette="cyan">End at {formatDate(endAt)}</Badge>
              )}
            </>
          )}
        </Flex>
