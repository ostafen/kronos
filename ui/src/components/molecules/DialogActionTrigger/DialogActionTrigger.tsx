...
export default function DialogActionTrigger(
  props: PropsWithChildren<DeleteScheduleTriggerProps>
) {
  const { children, onSuccess, onConfirm, dialogData, ...iconButtonProps } =
    props;
  ...
  const initFlow = async () => {
    ...
    try {
      await new Promise((resolve, reject) => {
        const confirmSub = dialogConfirm$.source$.subscribe(() => {
  ...
  return (
    <Button onClick={initFlow} {...iconButtonProps}>
      {children}
    </Button>
  );
}
